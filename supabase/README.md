...
La persistencia remota de catálogo ya está conectada. Las salas usan canales Realtime efímeros: el código determina el canal y el host reenvía el estado actual cuando se une un estudiante. Al finalizar un quiz en vivo, se guarda en `quiz_sessions` un snapshot con el quiz, asignatura, sala, fecha, clasificación y conteos agregados por pregunta. No se guardan identificadores de dispositivo ni respuestas individuales. Desde el historial, el docente puede exportar resultados o estadísticas por pregunta en CSV. El historial solo se muestra a la cuenta docente autorizada; las sesiones abandonadas y la práctica individual no se registran.
