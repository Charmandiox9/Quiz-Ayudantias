...
import type { CatalogSubject, EditorQuestion, QuizCatalog, QuizDefinition, QuizForm } from "../types";

const STORAGE_KEY = "quiz-ayudantias-catalog-v1";
const DEFAULT_SUBJECT_ID = "subject-ingenieria-software";

function createInitialCatalog(): QuizCatalog {
  return {
    version: 1,
    subjects: [
      {
...
function isCatalog(value: unknown): value is QuizCatalog {
  if (!value || typeof value !== "object") return false;
  const candidate = value as { version?: unknown; subjects?: unknown };
  return (
    candidate.version === 1 &&
    Array.isArray(candidate.subjects) &&
    candidate.subjects.every((subject: unknown) =>
      Boolean(subject && typeof subject === "object" && "quizzes" in subject && Array.isArray(subject.quizzes))
    )
  );
}

export function loadQuizCatalog(): QuizCatalog {
...
export function saveQuizCatalog(catalog: QuizCatalog): void {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(catalog));
}

export function createSubject({ name, code = "", description = "", sealLogoUrl = "" }: Pick<CatalogSubject, "name" | "code" | "description" | "sealLogoUrl">): CatalogSubject {
...
export function createQuiz({ subjectId, title, description = "", cardTitle = "", cardSubtitle = "", cardImage = "", practiceEnabled = false, questions }: QuizForm & { subjectId: string }): QuizDefinition {
  const cleanTitle = title.trim();
  const cleanQuestions = questions.map((question: EditorQuestion) => ({
    ...question,
...
export function findQuiz(catalog: QuizCatalog, quizId: string): { subject: CatalogSubject; quiz: QuizDefinition } | null {
...
export function publishQuiz(catalog: QuizCatalog, quizId: string): QuizCatalog {
  return {
    ...catalog,
...
export function archiveQuiz(catalog: QuizCatalog, quizId: string): QuizCatalog {
  return {
    ...catalog,
    subjects: catalog.subjects.map((subject) => ({
      ...subject,
...
export function restoreQuiz(catalog: QuizCatalog, quizId: string): QuizCatalog {
  return {
    ...catalog,
    subjects: catalog.subjects.map((subject) => ({
      ...subject,
...
export function addSubject(catalog: QuizCatalog, subject: CatalogSubject): QuizCatalog {
  return { ...catalog, subjects: [...catalog.subjects, subject] };
}

export function addQuiz(catalog: QuizCatalog, subjectId: string, quiz: QuizDefinition): QuizCatalog {
  return {
    ...catalog,
    subjects: catalog.subjects.map((subject) =>
...
export function updateQuiz(catalog: QuizCatalog, quizId: string, form: QuizForm): QuizCatalog {
