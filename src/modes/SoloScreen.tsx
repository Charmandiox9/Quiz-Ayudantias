...
export default function SoloScreen({ ayudantia, onExit }: { ayudantia: QuizDefinition; onExit: () => void }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedOptionIndex, setSelectedOptionIndex] = useState<SelectedAnswer>(null);
  const [orderingOrder, setOrderingOrder] = useState<number[]>(() => {
    const firstQuestion = ayudantia.questions[0];
    return firstQuestion && isOrdering(firstQuestion) ? shuffleIndices(firstQuestion.opts.length) : [];
  });
  const [isAnswerRevealed, setIsAnswerRevealed] = useState(false);
  const [score, setScore] = useState(0);
  const [correctAnswersCount, setCorrectAnswersCount] = useState(0);
  const [answersHistory, setAnswersHistory] = useState<AnswerHistoryEntry[]>([]);
  ...
  const handleSubmitAnswer = (answer: SelectedAnswer = isOrdering(currentQuestion) ? orderingOrder : selectedOptionIndex) => {
  ...
  const handleSelectAnswer = (answer: SelectedAnswer) => {
    ...
    if (isMultipleSelect(currentQuestion)) {
      setSelectedOptionIndex((previous) => {
        const selected = Array.isArray(previous) ? previous.filter((value): value is number => typeof value === "number") : [];
        return selected.includes(answer)
          ? selected.filter((item) => item !== answer)
          : [...selected, answer].sort((a, b) => a - b);
      });
      return;
    }
  ...
  const handleNext = () => {
    ...
    setOrderingOrder(nextQuestion && isOrdering(nextQuestion) ? shuffleIndices(nextQuestion.opts.length) : []);
  };
  ...
  const handleRestart = () => {
    ...
    setOrderingOrder(firstQuestion && isOrdering(firstQuestion) ? shuffleIndices(firstQuestion.opts.length) : []);
    setIsAnswerRevealed(false);
    setScore(0);
