...
export default function RewardCard({
  ...
  const [customSrcOverride, setCustomSrcOverride] = useState<string | null>(null);
   ...
   */
  const handleEnvelopePointerMove = (e: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
    const element = e.currentTarget;
    const rect = element.getBoundingClientRect();
    const touch = "touches" in e ? e.touches[0] : undefined;
    const clientX = "clientX" in e ? e.clientX : touch?.clientX ?? rect.left + rect.width / 2;
    const clientY = "clientY" in e ? e.clientY : touch?.clientY ?? rect.top + rect.height / 2;
   ...
   */
  const handleCardPointerMove = (e: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
  ...
  const handleImgError = () => {
    if (!["/assets/ay03_solid.png", "/assets/ay02_uml.png", "/favicon.png"].includes(activeImgSrc)) {
   ...
   */
  const saveImageFile = async (pngBlob: Blob, filename: string): Promise<boolean> => {
    const showSaveFilePicker = (window as SaveFilePickerWindow).showSaveFilePicker;
    if (typeof window !== "undefined" && showSaveFilePicker) {
      try {
