...
import type { PlayerInfo, QuizCatalog, QuizDefinition } from "./types";

type AppSession =
  | { role: "player"; playerInfo: PlayerInfo }
  | { role: "host"; ayudantia: QuizDefinition; roomCode: string; quizVersion: number }
  | { role: "solo"; ayudantia: QuizDefinition; quizVersion: number }
  | null;

interface InitialAppState { view: "hub" | "player"; session: PlayerInfo | null }
...
function getInitialState(): InitialAppState {
...
function freezeQuizSnapshot(quiz: QuizDefinition): QuizDefinition {
  const snapshot = JSON.parse(JSON.stringify(quiz)) as QuizDefinition;
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<"hub" | "host" | "player" | "solo">(initial.view);
  const [sessionData, setSessionData] = useState<AppSession>(() => initial.session ? { role: "player", playerInfo: initial.session } : null);
  ...
  const [publicCatalog, setPublicCatalog] = useState<QuizCatalog | null>(null);
  ...
  const handleStartHost = ({ ayudantia, roomCode }: { ayudantia: QuizDefinition; roomCode: string }) => {
    const quizSnapshot = freezeQuizSnapshot(ayudantia);
    setSessionData({ role: "host", ayudantia: quizSnapshot, roomCode, quizVersion: quizSnapshot.version || 1 });
    setCurrentView("host");
  };

  const handleJoinPlayer = ({ name, roomCode, ayudantia }: { name: string; roomCode: string; ayudantia?: QuizDefinition }) => {
    ...
    setSessionData({ role: "player", playerInfo: session });
    setCurrentView("player");
  };

  const sendTeacherLink = async (email: string): Promise<void> => {
  ...
  const handleSaveCatalog = async (nextCatalog: QuizCatalog): Promise<void> => {
  ...
  const handleStartSolo = ({ ayudantia }: { ayudantia: QuizDefinition }) => {
    const quizSnapshot = freezeQuizSnapshot(ayudantia);
    setSessionData({ role: "solo", ayudantia: quizSnapshot, quizVersion: quizSnapshot.version || 1 });
    setCurrentView("solo");
  };
  ...
  return (
    <div>
      ...
      {currentView === "host" && sessionData?.role === "host" && (
        <HostScreen
      ...
      {currentView === "player" && sessionData?.role === "player" && (
        <PlayerScreen
          playerInfo={sessionData.playerInfo}
          onExit={handleExitToHub}
        />
      )}

      {currentView === "solo" && sessionData?.role === "solo" && (
        <SoloScreen
          ayudantia={sessionData.ayudantia}
          onExit={handleExitToHub}
        />
