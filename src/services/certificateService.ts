...
interface CertificateDownload { serial_id: string; quiz_title: string; accuracy: number; score: number; player_nickname: string; downloaded_at: string }

function isCertificateDownload(value: unknown): value is CertificateDownload {
  return Boolean(value && typeof value === "object" && "serial_id" in value && typeof value.serial_id === "string");
}
...
export const certificateService = {
   ...
   */
  async recordDownload({ serialId, quizTitle, accuracy, score, nickname = "Estudiante" }: { serialId: string; quizTitle: string; accuracy: number; score: number; nickname?: string }): Promise<CertificateDownload> {
    ...
    try {
      ...
      const existing = Array.isArray(parsed) ? parsed.filter(isCertificateDownload) : [];
   ...
   */
  getLocalDownloads(): CertificateDownload[] {
    try {
      const parsed: unknown = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]");
      return Array.isArray(parsed) ? parsed.filter(isCertificateDownload) : [];
    } catch {
      return [];
    }
  },
